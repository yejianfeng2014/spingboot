...
<html>
<head>
	<title>email_sell_response_end</title>
...
<body style="background-color: #FFFFFF;">
	<div id="emailSellResponseEndList" v-cloak>
		<!--工具条-->
		<el-row style="margin-top: 15px;">
			<el-form :inline="true" :model="filters" size="mini" ref="filters">
				<el-form-item>
			    	<el-button type="primary" icon="el-icon-search" v-on:click="getEmailSellResponseEnds">查询</el-button>
			    </el-form-item>
			    <el-form-item>
		...
		<!--列表-->
		<el-table :data="emailSellResponseEnds" border stripe size="mini" highlight-current-row v-loading="listLoading" @sort-change="handleSortChange"  @selection-change="selsChange" style="width: 100%;">
...
<script>
	var vue = new Vue({			
		el:"#emailSellResponseEndList",
		data() {
			return {
				filters: {
				},
				url:{
					list:'${webRoot}/emailSellResponseEndController/datagrid?',
					del:'${webRoot}/emailSellResponseEndController/doDel?',
					batchDel:'${webRoot}/emailSellResponseEndController/doBatchDel?',
					queryDict:'${webRoot}/systemController/typeListJson',
					save:'${webRoot}/emailSellResponseEndController/doAdd?',
					edit:'${webRoot}/emailSellResponseEndController/doUpdate?',
					upload:'${webRoot}/systemController/filedeal.do',
					downFile:'${webRoot}/img/server/',
					exportXls:'${webRoot}/emailSellResponseEndController/exportXls&id=',
					ImportXls:'${webRoot}/emailSellResponseEndController/upload'
				},
				emailSellResponseEnds: [],
				total: 0,
				page: 1,
				pageSize:10,
				sort:{
		...
		methods: {
			...
			getEmailSellResponseEnds() {
				...
				this.$http.get(this.url.list,para).then((res) => {
					...
					this.emailSellResponseEnds = datas;
			...
			//导入
			ImportXls: function(){
				openuploadwin('Excel导入',this.url.ImportXls, "emailSellResponseEndList");
			},
			//初始化数据字典
