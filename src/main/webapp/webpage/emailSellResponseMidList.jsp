...
<html>
<head>
	<title>email_sell_response_mid</title>
...
<body style="background-color: #FFFFFF;">
	<div id="emailSellResponseMidList" v-cloak>
		<!--工具条-->
		<el-row style="margin-top: 15px;">
			<el-form :inline="true" :model="filters" size="mini" ref="filters">
				<el-form-item>
			    	<el-button type="primary" icon="el-icon-search" v-on:click="getEmailSellResponseMids">查询</el-button>
			    </el-form-item>
			    <el-form-item>
		...
		<!--列表-->
		<el-table :data="emailSellResponseMids" border stripe size="mini" highlight-current-row v-loading="listLoading" @sort-change="handleSortChange"  @selection-change="selsChange" style="width: 100%;">
			<el-table-column type="selection" width="55"></el-table-column>
			<el-table-column type="index" width="60"></el-table-column>
			<el-table-column prop="contentType" label="contentType" min-width="120" sortable="custom" show-overflow-tooltip></el-table-column>
			<el-table-column prop="contentCn" label="contentCn" min-width="120" sortable="custom" show-overflow-tooltip></el-table-column>
			<el-table-column prop="contentResponse" label="contentResponse" min-width="120" sortable="custom" show-overflow-tooltip></el-table-column>
		...
		<el-dialog :title="formTitle" fullscreen z-index="800" :visible.sync="formVisible" :close-on-click-modal="false">
			<el-form :model="form" label-width="80px" :rules="formRules" ref="form" size="mini">
					<el-form-item label="contentType" prop="contentType">
						<el-input v-model="form.contentType" auto-complete="off" placeholder="请输入contentType"></el-input>
					</el-form-item>
					<el-form-item label="contentCn" prop="contentCn">
						<el-input v-model="form.contentCn" auto-complete="off" placeholder="请输入contentCn"></el-input>
					</el-form-item>
					<el-form-item label="contentResponse" prop="contentResponse">
						<el-input v-model="form.contentResponse" auto-complete="off" placeholder="请输入contentResponse"></el-input>
					</el-form-item>
			</el-form>
...
<script>
	var vue = new Vue({			
		el:"#emailSellResponseMidList",
		data() {
			return {
				filters: {
				},
				url:{
					list:'${webRoot}/emailSellResponseMidController/datagrid?',
					del:'${webRoot}/emailSellResponseMidController/doDel?',
					batchDel:'${webRoot}/emailSellResponseMidController/doBatchDel?',
					queryDict:'${webRoot}/systemController/typeListJson',
					save:'${webRoot}/emailSellResponseMidController/doAdd?',
					edit:'${webRoot}/emailSellResponseMidController/doUpdate?',
					upload:'${webRoot}/systemController/filedeal.do',
					downFile:'${webRoot}/img/server/',
					exportXls:'${webRoot}/emailSellResponseMidController/exportXls&id=',
					ImportXls:'${webRoot}/emailSellResponseMidController/upload'
				},
				emailSellResponseMids: [],
				total: 0,
				page: 1,
				pageSize:10,
				sort:{
		...
		methods: {
			...
			//获取用户列表
			getEmailSellResponseMids() {
				var fields=[];
				fields.push('id');
				fields.push('id');
				fields.push('contentType');
				fields.push('contentCn');
				fields.push('contentResponse');
				let para = {
					params: {
						page: this.page,
						rows: this.pageSize,
				...
				this.$http.get(this.url.list,para).then((res) => {
					...
					this.emailSellResponseMids = datas;
			...
			//导入
			ImportXls: function(){
				openuploadwin('Excel导入',this.url.ImportXls, "emailSellResponseMidList");
			},
			//初始化数据字典
