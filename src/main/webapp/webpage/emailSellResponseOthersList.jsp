...
<html>
<head>
	<title>email_sell_response_others</title>
...
<body style="background-color: #FFFFFF;">
	<div id="emailSellResponseOthersList" v-cloak>
		<!--工具条-->
		<el-row style="margin-top: 15px;">
			<el-form :inline="true" :model="filters" size="mini" ref="filters">
				<el-form-item>
			    	<el-button type="primary" icon="el-icon-search" v-on:click="getEmailSellResponseOtherss">查询</el-button>
			    </el-form-item>
			    <el-form-item>
		...
		<!--列表-->
		<el-table :data="emailSellResponseOtherss" border stripe size="mini" highlight-current-row v-loading="listLoading" @sort-change="handleSortChange"  @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="content" label="content" min-width="120" sortable="custom" show-overflow-tooltip></el-table-column>
		...
		<el-dialog :title="formTitle" fullscreen z-index="800" :visible.sync="formVisible" :close-on-click-modal="false">
			<el-form :model="form" label-width="80px" :rules="formRules" ref="form" size="mini">
					...
					<el-form-item label="content" prop="content">
						<el-input v-model="form.content" auto-complete="off" placeholder="请输入content"></el-input>
					</el-form-item>
			</el-form>
...
<script>
	var vue = new Vue({			
		el:"#emailSellResponseOthersList",
		data() {
			return {
				filters: {
				},
				url:{
					list:'${webRoot}/emailSellResponseOthersController/datagrid?',
					del:'${webRoot}/emailSellResponseOthersController/doDel?',
					batchDel:'${webRoot}/emailSellResponseOthersController/doBatchDel?',
					queryDict:'${webRoot}/systemController.do?typeListJson',
					save:'${webRoot}/emailSellResponseOthersController/doAdd?',
					edit:'${webRoot}/emailSellResponseOthersController/doUpdate?',
					upload:'${webRoot}/systemController/filedeal.do',
					downFile:'${webRoot}/img/server/',
					exportXls:'${webRoot}/emailSellResponseOthersController.do?exportXls&id=',
					ImportXls:'${webRoot}/emailSellResponseOthersController.do?upload'
				},
				emailSellResponseOtherss: [],
				total: 0,
				page: 1,
				pageSize:10,
				sort:{
		...
		methods: {
			handleSortChange(sort){
				...
				this.getEmailSellResponseOtherss();
			},
			...
			getEmailSellResponseOtherss() {
				...
				this.$http.get(this.url.list,para).then((res) => {
					...
					this.emailSellResponseOtherss = datas;
			...
			//导入
			ImportXls: function(){
				openuploadwin('Excel导入',this.url.ImportXls, "emailSellResponseOthersList");
			},
			//初始化数据字典
