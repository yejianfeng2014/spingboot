...
<body style="background-color: #FFFFFF;">
	<div id="intentPhraseList" v-cloak>
		<!--工具条-->
		<el-row style="margin-top: 15px;">
			<el-form :inline="true" :model="filters" size="mini" ref="filters">
				<el-form-item>
			    	<el-button type="primary" icon="el-icon-search" v-on:click="getIntentPhrases">查询</el-button>
			    </el-form-item>
			    <el-form-item>
		...
		<el-dialog :title="formTitle" fullscreen z-index="800" :visible.sync="formVisible" :close-on-click-modal="false">
			<el-form :model="form" label-width="80px" :rules="formRules" ref="form" size="mini">
					<el-form-item label="意图id,来自intent表的主键" prop="intentId">
						<el-input v-model="form.intentId" auto-complete="off" placeholder="请输入意图id,来自intent表的主键"></el-input>
					</el-form-item>
					<el-form-item label="意图语句" prop="intentPhrase">
						<el-input v-model="form.intentPhrase" auto-complete="off" placeholder="请输入意图语句"></el-input>
					</el-form-item>
					<el-form-item label="短语描述" prop="intentDetail">
						<el-input v-model="form.intentDetail" auto-complete="off" placeholder="请输入短语描述"></el-input>
					</el-form-item>
			</el-form>
...
<script>
	var vue = new Vue({			
		...
		methods: {
			...
			//导入
			ImportXls: function(){
				openuploadwin('Excel导入',this.url.ImportXls, "intentPhraseList");
			},
			//初始化数据字典
